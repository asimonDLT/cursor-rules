...
def get_filesystem_domains(rules_dir: Path, correlation_id: str) -> set[str]:
    ...
    if not rules_dir.exists():
        logger.warning(
            f"[{correlation_id}] Rules directory does not exist: {rules_dir}"
        )
        return set()
    ...
    logger.info(
        f"[{correlation_id}] Found {len(domains)} filesystem domains: {sorted(domains)}"
    )
    return domains
...
def get_registry_domains(
    ...
    logger.info(
        f"[{correlation_id}] Found {len(domain_mappings)} domain_mappings: {sorted(domain_mappings)}"
    )
    logger.info(
        f"[{correlation_id}] Found {len(domain_metadata)} domain_metadata: {sorted(domain_metadata)}"
    )
...
def validate_domain_consistency(
    ...
    missing_filesystem_mappings = (
        registry_mappings - filesystem_domains
    ) - technical_domains
    if missing_filesystem_mappings:
        for domain in sorted(missing_filesystem_mappings):
            errors.append(
                f"Organizational domain '{domain}' in domain_mappings but has no filesystem directory"
            )

    missing_filesystem_metadata = (
        registry_metadata - filesystem_domains
    ) - technical_domains
    if missing_filesystem_metadata:
        for domain in sorted(missing_filesystem_metadata):
            errors.append(
                f"Organizational domain '{domain}' in domain_metadata but has no filesystem directory"
            )
    ...
    if mappings_only:
        for domain in sorted(mappings_only):
            errors.append(
                f"Domain '{domain}' in domain_mappings but not in domain_metadata"
            )

    if metadata_only:
        for domain in sorted(metadata_only):
            errors.append(
                f"Domain '{domain}' in domain_metadata but not in domain_mappings"
            )
    ...
    if technical_in_registry:
        logger.info(
            f"[{correlation_id}] Technical domains (role-based, no filesystem): {sorted(technical_in_registry)}"
        )
    ...
    if is_valid:
        ...
        logger.error(
            f"[{correlation_id}] Domain consistency validation failed with {len(errors)} errors"
        )
...
def validate_domain_metadata_schema(
    ...
    for domain_name, metadata in domain_metadata.items():
        ...
        if not status:
            ...
            errors.append(
                f"Domain '{domain_name}' has invalid status '{status}'. Must be one of: {valid_statuses}"
            )
        ...
        if last_reviewed:
            if not re.match(r"^\d{4}-\d{2}-\d{2}$", last_reviewed):
                errors.append(
                    f"Domain '{domain_name}' has invalid last_reviewed format '{last_reviewed}'. Must be YYYY-MM-DD"
                )
        ...
        if not template_type:
            errors.append(
                f"Domain '{domain_name}' missing required 'template_type' field"
            )
    ...
    if is_valid:
        ...
        logger.error(
            f"[{correlation_id}] Domain metadata schema validation failed with {len(errors)} errors"
        )
...
def validate_required_sections(
    ...
    for domain_name in filesystem_domains:
        ...
        if not mdc_files:
            errors.append(
                f"Domain '{domain_name}' has no .mdc files but has required_sections defined"
            )
            continue
    ...
    if is_valid:
        ...
        logger.error(
            f"[{correlation_id}] Required sections validation failed with {len(errors)} errors"
        )
...
def display_validation_summary(
    ...
    table.add_row(
        "Filesystem",
        str(len(filesystem_domains)),
        ", ".join(sorted(filesystem_domains)),
    )
    table.add_row(
        "domain_mappings",
        str(len(registry_mappings)),
        ", ".join(sorted(registry_mappings)),
    )
    table.add_row(
        "domain_metadata",
        str(len(registry_metadata)),
        ", ".join(sorted(registry_metadata)),
    )
...
def main() -> None:
    ...
    registry_mappings, registry_metadata = get_registry_domains(
    ...
    schema_valid, schema_errors = validate_domain_metadata_schema(
        registry, correlation_id
    )
