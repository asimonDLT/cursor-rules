...
from common.config import CursorRulesConfig
...
class RoleFactory:
    ...
    def _load_tool_registry(self) -> dict[str, Any]:
        ...
        try:
            ...
            if not registry_path.exists():
                if self.batch_mode:
                    raise RoleFactoryError(
                        f"Tool registry not found at {registry_path}"
                    )
            ...
            if not library_path.exists():
                ...
                if backup_path.exists():
                    if self.batch_mode and not self.force_mode:
                        raise RoleFactoryError(
                            f"Role library not found at {library_path}. Use --force to migrate from backup."
                        )

                    if self.force_mode or self._confirm_migration(
                        backup_path, library_path
                    ):
                    ...
                    raise RoleFactoryError(
                        f"Role library not found at {library_path} and no backup available"
                    )
                ...
                for category in categories:
                    if category in tool_categories:
                        resolved_tools.extend(
                            tool_categories[category].get("tools", [])
                        )
    ...
    def validate_cli_input(self, value: str, field_name: str) -> bool:
        ...
        if any(pattern in value.lower() for pattern in DANGEROUS_INPUT_PATTERNS):
            ...
            console.print(
                f"[red]Error:[/red] Invalid characters detected in {field_name}"
            )
            return False
    ...
    def validate_role_name(self, name: str) -> str:
        ...
        if not sanitized:
            raise RoleFactoryError(
                f"Invalid role name: {name} - must contain alphanumeric characters"
            )
    ...
    def get_role_data(self, role_type: str, role_name: str) -> dict[str, Any] | None:
        """Get role data from library with improved handling."""
        if role_type not in self.role_library:
            raise RoleFactoryError(
                f"Unknown role type: {role_type}. Available types: {list(self.role_library.keys())}"
            )
        ...
        if self.batch_mode and not self.force_mode:
            raise RoleFactoryError(
                f"Role '{role_name}' not in library. Use --force to create custom role."
            )
    ...
    def run_validation(self, output_path: Path) -> None:
        ...
        try:
            mdc_linter_path = (
                self.config.get_path("scripts") / "validation" / "lint_mdc.py"
            )
            if mdc_linter_path.exists():
                ...
                console.print("[yellow]⚠ MDC linter not found[/yellow]")
...
Examples:
  ...
  uv run python create_role_v2.py --name data_architect --type specialist --tool-domains analytics,data --batch
    ...
    parser.add_argument(
        "--force", action="store_true", help="Force operations, bypass confirmations"
    )
    parser.add_argument(
        "--strict", action="store_true", help="Fail if required data is missing"
    )
    ...
    parser.add_argument(
        "--tool-domains", help="Comma-separated list of tool domains from registry"
    )
    ...
    parser.add_argument(
        "--validate-config",
        action="store_true",
        help="Validate configuration and paths",
    )
    parser.add_argument(
    ...
    try:
        ...
        if args.validate_config or args.list_templates:
            ...
            console.print(
                "[red]Error:[/red] --name and --type are required for role creation"
            )
            parser.print_help()
            sys.exit(1)
        ...
        if args.list_templates:
            ...
            for role_type, roles in factory.role_library.items():
                ...
                for role_name, role_data in roles.items():
                    frameworks = role_data.get(
                        "frameworks", role_data.get("standards", [])
                    )
        ...
        if not args.batch:
            console.print(
                "[dim]This is the improved v2 implementation. Use the original create_role.py for full functionality.[/dim]"
            )
